...
export const adminPrivateDailyImports = pgTable(
  "admin_private_daily_imports",
  {
    id: serial().primaryKey(),
    date: text().notNull(),
...
// idempotent.
export const hiddenShifts = pgTable(
  "hidden_shifts",
  {
    id: serial().primaryKey(),
    date: text().notNull(), // ISO date 'YYYY-MM-DD'
...
export const availability = pgTable(
  ...
  (t) => [
    uniqueIndex("availability_user_date_shift_idx").on(
      t.userId,
      t.date,
      t.shiftType,
    ),
...
export const formCompletions = pgTable(
  "form_completions",
  {
    id: serial().primaryKey(),
    date: text().notNull(), // ISO date 'YYYY-MM-DD'
  ...
  (t) => [
    uniqueIndex("form_completions_date_source_ref_slot_idx").on(
      t.date,
      t.source,
      t.refId,
      t.slot,
...
export const publishedWeeks = pgTable(
  "published_weeks",
  {
    id: serial().primaryKey(),
...
export const placementNotes = pgTable(
  "placement_notes",
  {
    id: serial().primaryKey(),
